...
if vim.g.neovide then
  ...
  function show_scale_factor()
    last_scale_notify_id = vim.notify(string.format('Scale: %.2f', vim.g.neovide_scale_factor),
      vim.log.levels.INFO,
      { replace = last_scale_notify_id }
    )
  end
  ...
  map({ 'n', 'i' }, '<S-A-ScrollWheelUp>', increase_scale_factor, { desc = 'Zoom in', silent = true })
  map({ 'n', 'i' }, '<S-A-ScrollWheelDown>', decrease_scale_factor, { desc = 'Zoom out', silent = true })
